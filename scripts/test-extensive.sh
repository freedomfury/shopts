...
# Extensive test for go-getopt: covers all supported types, variables, and edge cases
...
list_delim_val=""
while IFS= read -r -d $'\0' k && IFS= read -r v; do
  if [[ "${k}" == "GO_GETOPT_LISTVAL" ]]; then
    list_delim_val="${v}"
    break
  fi
done < <(GO_GETOPT_LIST_DELIM=":" "${binary}" "${SCHEMA}" -s test -l a -l b -l c)
